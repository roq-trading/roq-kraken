...
Random::Random(
    ...
    : _key(key), _password(password), _hmac(create_hmac(secret)) {
}

std::string Random::create_body() {
  auto now =
      std::chrono::duration_cast<decltype(_nonce)>(core::get_realtime_clock());
  ...
  if (diff < 0)
    ...
    _nonce = now;
  if (_password.empty()) {
    return fmt::format(R"(nonce={})", _nonce.count());
  } else {
    return fmt::format(
        R"("nonce={}&)"
        R"("opt={}")",
...
std::string Random::create_headers(
  ...
  auto nonce = fmt::format("{}", _nonce.count());
  _sha.clear();
  _sha.update(nonce);
  _sha.update(body);
  std::array<char, 32> buffer_1;
  auto length_1 = _sha.digest(buffer_1.data(), buffer_1.size());
  ...
  auto length_2 = _hmac.digest(buffer_2.data(), buffer_2.size());
